...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.title("🧬 Predictive Modeling — Defect Classification")

    st.markdown("""
    Model klasifikasi untuk memprediksi apakah suatu sesi produksi akan menghasilkan cacat (`defect_count > 0`).
    """)

    with st.spinner("Training model... harap tunggu sebentar ⏳"):
        # Prepare target
        df_model = df.copy()
        df_model['has_defect'] = (df_model['defect_count'] > 0).astype(int)
        ...
        # Add shift_efficiency_cluster if available
        if 'shift_efficiency_cluster' in df_model.columns:
            feature_cols.append('shift_efficiency_cluster')

        X = df_model[feature_cols].dropna()
        y = df_model.loc[X.index, 'has_defect']
    ...
    # Metrics
    col1, col2, col3 = st.columns(3)
    col1.metric("Accuracy (RF)",  f"{acc_rf:.2%}")
    col2.metric("F1-Score (RF)",  f"{f1_rf:.3f}")
    col3.metric("ROC-AUC (RF)",   f"{auc_rf:.3f}")

    tab1, tab2 = st.tabs(["📊 Classification Report", "🌟 Feature Importance"])

    with tab1:
        report = classification_report(y_test, y_pred_rf, output_dict=True)
        report_df = pd.DataFrame(report).T
        st.dataframe(report_df.style.background_gradient(cmap='Blues', subset=['f1-score','precision','recall']),
    ...
    if XGBOOST_AVAILABLE:
        ...
        col_a.metric("Accuracy (XGB)", f"{accuracy_score(y_test, y_pred_xgb):.2%}")
        col_b.metric("F1-Score (XGB)", f"{f1_score(y_test, y_pred_xgb):.3f}")
        col_c.metric("ROC-AUC (XGB)",  f"{roc_auc_score(y_test, y_prob_xgb):.3f}")
